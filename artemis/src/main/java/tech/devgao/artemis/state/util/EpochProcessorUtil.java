...
public class EpochProcessorUtil {
  ...
  public static void updateValidatorRegistry(BeaconState state){
    ...
    double max_balance_churn = Math.max((double)(Constants.MAX_DEPOSIT * Constants.GWEI_PER_ETH),
    ...
    double total_penalties = latest_penalized_exit_balances.get(period_index) +
  ...
  private static void updatePendingValidators(double max_balance_churn, BeaconState state){
    ...
    for (ValidatorRecord validator : state.getValidator_registry()) {
      if (validator.getStatus().getValue() == Constants.PENDING_ACTIVATION && validator.getBalance() >=
        ...
        state.update_validator_status(state, state.getValidator_registry().indexOf(validator),
            Constants.ACTIVE);
      }
    }
  }
  ...
  private static void updateActivePendingExit(double max_balance_churn, BeaconState state){
    ...
    for (ValidatorRecord validator : state.getValidator_registry()) {
      if (validator.getStatus().getValue() == Constants.ACTIVE_PENDING_EXIT && validator.getBalance() >=
          Constants.MAX_DEPOSIT * Constants.GWEI_PER_ETH){
        balance_churn += validator.get_effective_balance();
