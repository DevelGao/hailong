...
import net.develgao.beaconchain.crypto.Hash;
import net.develgao.beaconchain.ethereum.core.SealableBlockHeader;
import net.develgao.beaconchain.ethereum.rlp.BytesValueRLPOutput;
...
public final class EthHash {
  ...
  private static final ThreadLocal<MessageDigest> KECCAK_256 = ThreadLocal.withInitial(() -> {
    try {
   ...
   *         bytes 32 to 63
   */
  public static byte[] hashimotoLight(final long size, final int[] cache, final byte[] header,
      final long nonce) {
    return hashimoto(header, size, nonce, (target, ind) -> calcDatasetItem(target, cache, ind));
  }

  public static byte[] hashimoto(final byte[] header, final long size, final long nonce,
    ...
    for (int i = 0; i < ACCESSES; ++i) {
      final int p = Integer.remainderUnsigned(
  ...
  public static byte[] hashHeader(final SealableBlockHeader header) {
    ...
    out.startList();
    out.writeBytesValue(header.parentHash());
    out.writeBytesValue(header.ommersHash());
    out.writeBytesValue(header.coinbase());
    out.writeBytesValue(header.stateRoot());
    out.writeBytesValue(header.transactionsRoot());
    out.writeBytesValue(header.receiptsRoot());
    out.writeBytesValue(header.logsBloom().bytes());
    out.writeUInt256Scalar(header.difficulty());
    out.writeLongScalar(header.number());
    out.writeLongScalar(header.gasLimit());
    out.writeLongScalar(header.gasUsed());
    out.writeLongScalar(header.timestamp());
    out.writeBytesValue(header.extraData());
    out.endList();
      ...
      for (int j = 0; j < rows; ++j) {
        ...
        for (int k = 0; k < HASH_BYTES; ++k) {
          temp[k] = (byte) (cache[(j - 1 + rows) % rows * HASH_BYTES + k]
              ^ cache[Integer.remainderUnsigned(readLittleEndianInt(cache, offset), rows)
                  * HASH_BYTES + k]);
        }
  ...
  private static int readLittleEndianInt(final byte[] buffer, final int offset) {
    return Ints.fromBytes(buffer[offset + 3], buffer[offset + 2], buffer[offset + 1],
        buffer[offset]);
  }
