...
class RLPEncodingHelpers {
  ...
  static boolean isSingleRLPByte(BytesValue value) {
    return value.size() == 1 && value.get(0) >= 0;
  }

  static boolean isShortElement(BytesValue value) {
   ...
   * enough).
   */
  static int writeElement(BytesValue value, MutableBytesValue dest, int destOffset) {
    int size = value.size();
    ...
    destOffset = writeLongMetadata(0xb7, size, dest, destOffset);
    value.copyTo(dest, destOffset);
    return destOffset + size;
  }
   ...
   */
  static int writeListHeader(int payloadSize, MutableBytesValue dest, int destOffset) {
