...
import tech.devgao.artemis.util.bytes.MutableBytesValue;
import tech.devgao.artemis.util.uint.UInt256Value;
 ...
 *
 * <p>
 * A {@link RLPOutput} thus provides methods to write both lists and binary values. A list is
 * started by calling {@link #startList()} and ended by {@link #endList()}. Lists can be nested in
 * other lists in arbitrary ways. Binary values can be written directly with
 * {@link #writeBytesValue(BytesValue)}, but the {@link RLPOutput} interface provides a wealth of
 * convenience methods to write specific types of data with a specific encoding.
 *
 * <p>
 * Amongst the methods to write binary data, some methods are provided to write "scalar". A scalar
 * should simply be understood as a positive integer that is encoded with no leading zeros. In other
 * word, if an integer is written with a "Scalar" method variant, that number will be encoded with
 * the minimum number of bytes necessary to represent it.
 *
 * <p>
 * The {@link RLPOutput} only defines the interface for writing data meant to be RLP encoded.
 * Getting the finally encoded output will depend on the concrete implementation, see
 * {@link BytesValueRLPOutput} for instance.
 */
public interface RLPOutput {
   ...
   *
   * <p>
   * This is a shortcut for {@code writeBytesValue(BytesValue.EMPTY)}.
   */
   ...
   */
  default void writeIntScalar(int v) {
    writeLongScalar(v);
  }
   ...
   * @throws IllegalArgumentException if {@code v < 0}.
   */
  default void writeLongScalar(long v) {
   ...
   */
  default void writeBigIntegerScalar(BigInteger v) {
    ...
    writeBytesValue(bytes.length > 1 && bytes[0] == 0 ? BytesValue.wrap(bytes, 1, bytes.length - 1)
   ...
   * @param v The scalar to write.
   */
  default void writeUInt256Scalar(UInt256Value<?> v) {
    writeBytesValue(BytesValues.trimLeadingZeros(v.bytes()));
  }
   ...
   *
   * <p>
   * Note that this is not a "scalar" write: the value will be encoded with exactly 2 bytes.
   *
   ...
   *
   * <p>
   * Note that this is not a "scalar" write: the value will be encoded with exactly 4 bytes.
   *
   * @param i The 4-bytes int to write.
   */
  default void writeInt(int i) {
    MutableBytesValue v = MutableBytesValue.create(4);
    v.setInt(0, i);
    writeBytesValue(v);
  }
   ...
   *
   * <p>
   * Note that this is not a "scalar" write: the value will be encoded with exactly 8 bytes.
   *
   * @param l The 8-bytes long to write.
   */
  default void writeLong(long l) {
    MutableBytesValue v = MutableBytesValue.create(8);
    v.setLong(0, l);
    writeBytesValue(v);
  }
   ...
   */
  default void writeUnsignedByte(int b) {
   ...
   */
  default void writeUnsignedShort(int s) {
   ...
   */
  default <T> void writeList(Collection<T> values, BiConsumer<T, RLPOutput> valueWriter) {
    startList();
    for (T v : values) {
      valueWriter.accept(v, this);
    }
   ...
   *
   * <p>
   * This method is the functional equivalent of decoding the provided value entirely (to an fully
   * formed Java object) and then re-encoding that result to this output. It is however a lot more
   * efficient in that it saves most of that decoding/re-encoding work. Please note however that
   * this method <b>does</b> validate that the input is a valid RLP encoding. If you can guaranteed
   * that the input is valid and do not want this validation step, please have a look at
   * {@link #writeRLPUnsafe(BytesValue)}.
   *
   * @param rlpEncodedValue An already RLP encoded value to write as next item of this output.
   */
  default void writeRLP(BytesValue rlpEncodedValue) {
   ...
   *
   * <p>
   * This method is equivalent to {@link #writeRLP(BytesValue)}, but is unsafe in that it does not
   * do any validation of the its input. As such, it is faster but can silently yield invalid RLP
   * output if misused.
   *
   * @param rlpEncodedValue An already RLP encoded value to write as next item of this output.
   */
