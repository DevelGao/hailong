...
import static tech.devgao.artemis.ethereum.rlp.RLPDecodingHelpers.extractSize;
import static tech.devgao.artemis.ethereum.rlp.RLPEncodingHelpers.elementSize;
import static tech.devgao.artemis.ethereum.rlp.RLPEncodingHelpers.isSingleRLPByte;
import static tech.devgao.artemis.ethereum.rlp.RLPEncodingHelpers.writeElement;

import tech.devgao.artemis.ethereum.rlp.RLPDecodingHelpers.Kind;
...
public abstract class RLP {
   ...
   *
   * <p>
   * The created input is strict, in that exceptions will be thrown for any malformed input, either
   * by this method or by future reads from the returned input.
   *
   ...
   *         (item that can be a list itself). Note that more deeply nested corruption/malformation
   *         of the input will not be detected by this method call, but will be later when the input
   *         is read.
   */
  public static RLPInput input(BytesValue encoded) {
   ...
   * @return A newly created {@link RLPInput} to decode RLP data in {@code encoded} from
   *         {@code offset}.
   * @throws MalformedRLPInputException if {@code encoded} doesn't contain a properly encoded RLP
   *         item. Note that this only detect malformation on the main item at {@code offset}, but
   *         more deeply nested corruption/malformation of the input will not be detected by this
   *         method call, but only later when the input is read.
   */
  public static VertxBufferRLPInput input(Buffer buffer, int offset) {
   ...
   *
   * <p>
   * This method is mostly intended for testing as it is often more convenient <b>and</b> efficient
   * to use a {@link RLPInput} (through {@link #input(BytesValue)}) instead.
   *
   ...
   *         or a list whose elements are either {@link BytesValue}, or similarly composed
   *         sub-lists.
  ...
  private static Object decode(RLPInput in) {
    if (!in.nextIsList()) {
   ...
   * <p>
   * This method is mostly intended for testing as it is often more convenient <b>and</b> efficient
   * to use a {@link RLPOutput} (through {@link #encode(Consumer)} for instance) instead.
   *
   ...
   */
  public static BytesValue encode(Object obj) {
    BytesValueRLPOutput out = new BytesValueRLPOutput();
    encode(obj, out);
    return out.encoded();
  }

  private static void encode(Object obj, RLPOutput out) {
   ...
   *
   * <p>
   * This method is a convenience method that is mostly meant for use with class that have a method
   * to write to an {@link RLPOutput}. For instance:
   *
   * <pre>
   * {@code
   *   class Foo {
   ...
   */
  public static BytesValue encode(Consumer<RLPOutput> writer) {
    BytesValueRLPOutput out = new BytesValueRLPOutput();
    writer.accept(out);
    return out.encoded();
  }
   ...
   */
  public static BytesValue encodeOne(BytesValue value) {
    if (isSingleRLPByte(value))
      return value;

    MutableBytesValue res = MutableBytesValue.create(elementSize(value));
   ...
   */
  public static BytesValue decodeOne(BytesValue encodedValue) {
    ...
    if (kind == Kind.SHORT_ELEMENT) {
      ...
      int sizeLength = prefix - 0xb7;
      if (1 + sizeLength > encodedValue.size()) {
        throw new RLPException(format(
    ...
    if (offset + size != encodedValue.size()) {
      throw new RLPException(format("Malformed RLP input: %s should be of size %d according to "
          + "prefix byte but of size %d", encodedValue, offset + size, encodedValue.size()));
    }
   ...
   */
  public static void validate(BytesValue encodedValue) {
    RLPInput in = input(encodedValue);
    while (!in.isDone()) {
      if (in.nextIsList()) {
        in.enterList();
