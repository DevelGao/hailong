...
usage() {
  ...
  echo "Example: Run multiple clients in interop mode using static peering. 16 validators and all are assigned to Hailong"
...
if [ "$START_HAILONG" = true ]
then

    export HAILONG_JAVA_VERSION=openjdk64-11.0.1
    SCRIPT_DIR=`pwd`
    ...
    tmux new-session -d -s foo "jenv local $HAILONG_JAVA_VERSION; cd $SCRIPT_DIR/demo/node_0/ && ./hailong --config=$CONFIG_DIR/runConfig.0.toml --logging=DEBUG; sleep 20"
fi
...
then
    ...
    export PORT=19001
    #TODO: use a relative path to lighthouse dir.  the best way would be to deploy it to $HAILONG_ROOT/scripts/demo/node_lighthouse
    export DIR=$HOME/projects/develgao/devgao/lighthouse/lighthouse/target/release
...
#PYTHONWARNINGS=ignore::DeprecationWarning trinity-beacon -l DEBUG --trinity-root-dir /tmp/bb --beacon-nodekey='aaaaaaaaaa' --port $PORT interop --start-time $GENESIS_TIME --wipedb --genesis $GENESIS_FILE
    ...
    export PORT=19002
    # Start Trinity
    export DIR=$HOME/projects/develgao/devgao/trinity/
...
then
    ...
    export PORT=19003
    export DIR=$HOME/projects/develgao/devgao/nim-beacon-chain/multinet
    rm -f $DIR/validators/*
...
then
    ...
    export PORT=19004
    export DIR=$HOME/projects/develgao/devgao/lodestar
...
then
    export PORT=19005
    export DIR=$HOME/projects/develgao/devgao/prysm/darwin_amd64
...
then
    ...
    export PORT=19006
    export DIR=$HOME/projects/develgao/devgao/beacon-chain-java/node-0.2.0/bin
