...
public class ValidatorRegistrationClient {
    ...
    public void listenToPoWChain(){
        ...
        eth1DepositSub = contract.eth1DepositEventObservable(filter).subscribe(response -> {
            validatorRegistered(response);
        });

        //Subscribe to the event when 2^14 validators have been registered in the ValidatorRegistrationContract
        chainStartSub = contract.chainStartEventObservable(filter).subscribe(response -> {
    ...
    public ValidatorRegistrationContract deployVRC(Web3j web3j, Credentials credentials){
        ...
        ValidatorRegistrationContract contract = null;
        try{
            contract = ValidatorRegistrationContract.deploy(
                    web3j,
                    credentials,
                    contractGasProvider
            ).send();
    ...
    public ValidatorRegistrationContract getDeployedVRC(Web3j web3j, Credentials credentials){
        ...
        ValidatorRegistrationContract contract = null;
        try{
            contract = ValidatorRegistrationContract.load(
                    rb.getString("vrc_contract_address"),
                    web3j,
                    credentials,
                    contractGasProvider
            );
