...
import static tech.devgao.hailong.util.config.Constants.ETH1_FOLLOW_DISTANCE;
import static tech.devgao.hailong.util.config.Constants.ETH1_REQUEST_BUFFER;
import static tech.devgao.hailong.util.config.Constants.SECONDS_PER_ETH1_BLOCK;
...
import tech.devgao.hailong.pow.event.CacheEth1BlockEvent;
import tech.devgao.hailong.util.EventSink;
import tech.devgao.hailong.util.async.SafeFuture;
import tech.devgao.hailong.util.async.StubAsyncRunner;
...
public class Eth1DataManagerTest {
  ...
  //    = (200 - 160) / (6)
  //    = 7 (here we use
  //    i.e. Hailong will assume the middle-range block has block number current head - 7
  ...
  void cacheStartup_recalculateSecondsToFindMidRangeBlock() {
    // There is ice-age! Average block times are at about 20 seconds! What does Hailong do? FUNCTION.
    List<MockBlock> eth1Blocks =
        Arrays.asList(
            new MockBlock(15, 78),
            new MockBlock(16, 98),
